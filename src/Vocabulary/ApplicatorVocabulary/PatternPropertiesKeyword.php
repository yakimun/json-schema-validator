...
final class PatternPropertiesKeyword implements Keyword
{
    ...
    public function process(array $properties, SchemaContext $context): void
    {
        ...
        foreach ($property->getProperties() as $key => $value) {
            $valueIdentifier = new SchemaIdentifier($uri, $fragment->addTokens('patternProperties', $key));
            $schemaValidators['/' . $key . '/'] = $context->createValidator($value, $valueIdentifier);
        }

        $context->addKeywordHandler(new PatternPropertiesKeywordHandler($schemaValidators));
    }
}
