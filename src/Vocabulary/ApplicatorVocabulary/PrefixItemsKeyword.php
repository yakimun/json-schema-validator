...
final class PrefixItemsKeyword implements Keyword
{
    ...
    public function process(array $properties, SchemaContext $context): void
    {
        ...
        foreach ($items as $index => $item) {
            $itemIdentifier = new SchemaIdentifier($uri, $fragment->addTokens('prefixItems', (string)$index));
            $schemaValidators[] = $context->createValidator($item, $itemIdentifier);
        }

        $context->addKeywordHandler(new PrefixItemsKeywordHandler($schemaValidators));
    }
}
